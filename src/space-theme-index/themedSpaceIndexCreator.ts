...
class ThemedSpaceIndexCreator {
  ...
  // buildThemesUsingAi = async (spaceKey: string, searchResults: ConfluenceContentSearchResult[]): Promise<SpaceThemeIndexCreationResult> => {
  //   const prompt = this._buildCreateIndexPrompt(searchResults);
  //   console.log(`Built prompt: ${prompt} (length ${prompt.length})`);
  //   const response = await openAiUtil.postChatCompletion(prompt);
  ...
  //       console.log(`response content length: ${firstChoice.message.content.length}`);
  //       const lines = firstChoice.message.content.split('\n') as string[];
  //       let currentGroup: undefined | ThemedSpaceIndexGroup = undefined;
  //       for (const line of lines) {
  //         console.info(`INFO: analysing line: ${line}`);
  //         const lineLower = line.toLowerCase();
  ...
  //     console.log(`response text: ${text} (length ${text.length})`);
  //     result.message = text;
  //   }
  //   return result;
  // }
